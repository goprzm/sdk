 ...
 */
export const bootstrapErrorGuardScript = `(()=>{try{var K="${BOOTSTRAP_RELOAD_KEY}";var s=window.sessionStorage;var P=[/importing a module script failed/i,/failed to fetch dynamically imported module/i,/dynamically imported module/i,/loading chunk \\d+ failed/i,/chunkloaderror/i,/failed to resolve module specifier/i];function m(e){if(!e)return false;var x=typeof e==="string"?e:(e&&e.message)||"";return P.some(function(p){return p.test(x);});}function h(e){var t=(e&&(e.reason||e.error||e.message))||e;if(!m(t))return;try{if(s&&s.getItem(K)==="1")return;if(s)s.setItem(K,"1");}catch(_){}try{window.location.reload();}catch(_){}}window.addEventListener("error",h,true);window.addEventListener("unhandledrejection",h);}catch(_){}})();`;
